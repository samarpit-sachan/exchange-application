...
export function SwapUI({ market }: { market: string }) {
    ...
    const handleBuySell = async () => {
        if (!amount || !quantity) {
            console.error('Amount and quantity must be provided');
            return;
        }
                ...
                <div className="flex flex-col px-3">
                    <div className="flex flex-col flex-1 gap-3 text-baseTextHighEmphasis">
                        <div className="flex flex-col gap-3">
                            <div className="flex items-center justify-between flex-row">
                                <p className="text-xs font-normal text-baseTextMedEmphasis text-white">Available Balance</p>
                                <p className="font-medium text-xs text-baseTextHighEmphasis text-white">36.94 USDC</p>
                            </div>
                        </div>
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="flex flex-col relative">
                                <input step="0.01" placeholder="0"
                                    className="h-12 rounded-lg border-2 border-solid border-white bg-[var(--background)] pr-12 text-right text-2xl leading-9 text-white placeholder-baseTextMedEmphasis ring-0 transition focus:border-accentBlue focus:ring-0"
                                    value={amount}
                                    onChange={(e) => setAmount(e.target.value)}
                                />
                                <div className="flex flex-row absolute right-1 top-1 p-2">
                                    <div className="relative">
                                        <img src="https://cryptologos.cc/logos/usd-coin-usdc-logo.png" className="w-6 h-6" alt="USDC logo" />
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <div className="flex flex-col relative">
                            ...
                            <div className="flex flex-row absolute right-1 top-1 p-2">
                                <div className="relative">
                                    <img src="https://cryptologos.cc/logos/solana-sol-logo.png" className="w-6 h-6" alt="Solana logo" />
                                </div>
                            </div>
                        </div>
                    ...
                    <button type="button"
                        ...
                        disabled={isLoading}
                    >
                        {isLoading ? 'Loading...' : activeTab=='buy'? 'Buy':'Sell'}
                    </button>
...
function LimitButton({ type, setType }: { type: string, setType: any }) {
    return <div className="flex flex-col cursor-pointer justify-center py-2" onClick={() => setType('limit')}>
        <div className={`text-sm font-medium py-1 border-b-2 text-white ${type === 'limit' ? "border-accentBlue" : "border-transparent hover:border-baseTextHighEmphasis"}`}>
            Limit
        </div>
    </div>
}
...
function MarketButton({ type, setType }: { type: string, setType: any }) {
    return <div className="flex flex-col cursor-pointer justify-center py-2" onClick={() => setType('market')}>
        <div className={`text-sm font-medium py-1 border-b-2 text-white ${type === 'market' ? "border-accentBlue" : "border-b-2 border-transparent hover:border-baseTextHighEmphasis"}`}>
            Market
        </div>
    </div>
}
